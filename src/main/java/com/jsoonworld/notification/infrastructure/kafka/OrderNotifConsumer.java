...
@Component
public class OrderNotifConsumer extends AbstractNotifConsumer {

    private static final String CONSUMER_GROUP = "notif-order";
    private final EventDeserializer eventDeserializer;

    public OrderNotifConsumer(ProcessedEventPort processedEventPort,
    ...
    @KafkaListener(
        topics = "fluxpay.order.events",
        groupId = CONSUMER_GROUP,
        containerFactory = "kafkaListenerContainerFactory"
    )
    public void consume(ConsumerRecord<String, String> record, Acknowledgment ack) {
        log.info("Received order event: topic={}, partition={}, offset={}",
    ...
    protected Mono<NotificationRequest> processEvent(ConsumerRecord<String, String> record) {
        return Mono.fromCallable(() -> {
            ...
            if (payload.has("orderStatus")) templateVars.put("orderStatus", payload.get("orderStatus").asText());
    ...
    private String resolveSubject(EventType eventType) {
        return switch (eventType) {
            case ORDER_CREATED -> "[moalog] 주문이 접수되었습니다";
            case ORDER_COMPLETED -> "[moalog] 주문이 완료되었습니다";
            case ORDER_CANCELLED -> "[moalog] 주문이 취소되었습니다";
            default -> "[moalog] 알림";
        };
    }
}
